"use client";

import { getProviders, signIn } from "next-auth/react";
import { useEffect, useState } from "react";

import { primaryButton, secondaryButton } from "@/lib/format";

function GitHubMark() {
  return (
    <svg viewBox="0 0 16 16" fill="currentColor" className="h-4 w-4" aria-hidden>
      <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82a7.42 7.42 0 0 1 4 0c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8Z" />
    </svg>
  );
}

export function SignInButtons() {
  const [providers, setProviders] = useState<Record<string, { id: string }> | null>(null);
  const [devEmail, setDevEmail] = useState("demo@beacon.dev");

  useEffect(() => {
    getProviders().then((p) => setProviders(p as Record<string, { id: string }> | null));
  }, []);

  if (!providers) {
    return <div className="h-11 w-56 animate-pulse rounded-md bg-zinc-200" />;
  }

  return (
    <div className="flex flex-col gap-3">
      {providers.github ? (
        <button
          type="button"
          onClick={() => signIn("github", { callbackUrl: "/home" })}
          className={`inline-flex h-11 w-fit items-center gap-2.5 px-6 ${primaryButton}`}
        >
          <GitHubMark />
          Sign in with GitHub
        </button>
      ) : null}
      {providers.dev ? (
        <form
          className="flex items-center gap-2"
          onSubmit={(event) => {
            event.preventDefault();
            signIn("dev", { email: devEmail, callbackUrl: "/home" });
          }}
        >
          <input
            type="email"
            required
            value={devEmail}
            onChange={(event) => setDevEmail(event.target.value)}
            className="h-10 w-52 rounded-md border border-edge bg-white px-3 text-sm text-zinc-800 outline-none focus:border-beacon"
            placeholder="you@example.com"
            aria-label="Dev sign-in email"
          />
          <button type="submit" className={`h-10 px-4 text-sm ${secondaryButton}`}>
            Dev sign-in
          </button>
        </form>
      ) : null}
      {!providers.github && !providers.dev ? (
        <p className="text-sm text-red-700">
          No auth providers configured — set GITHUB_CLIENT_ID or AUTH_DEV_MODE=true.
        </p>
      ) : null}
    </div>
  );
}
